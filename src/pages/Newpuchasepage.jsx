...
export default function NewPurchasePage() {
  ...
  const [transport, setTransport] = useState({ vehicleNo: '', dispatchFrom: '', dispatchThrough: 'Credit' })
  ...
  const computedItems = items.map((item) => {
    ...
    return {
      ...item, qty, rate, discountPct, grossAmount, discountAmount, baseAmount, taxPct, taxAmount, lineTotal,
    }
  })
  ...
  useEffect(() => {
    if (!isOpen) return
    const activeOption = supplierOptionRefs.current[highlightedIndex]
  ...
  const commitSupplierSelection = useCallback((party) => {
    applySupplier(party)
    focusCell(0, 'desc')
  }, [focusCell])
  ...
  useEffect(() => {
    ...
    document.addEventListener('keydown', handleGlobal, true)
    return () => document.removeEventListener('keydown', handleGlobal, true)
  }, [savePurchase])
  ...
  const consumeHeaderEnter = (index) => (event) => {
    ...
    const supplierInput = chain[0]

    if (index === 0) {
      if (handleKeyDown(event, commitSupplierSelection)) return
      const activeEl = document.activeElement
      if (!(activeEl instanceof HTMLElement) || !(supplierInput instanceof HTMLElement)) return
      if (activeEl !== supplierInput && !supplierInput.contains(activeEl)) return
      ...
      event.preventDefault()
      const exact = supplierParties.find((party) => party.name?.toLowerCase() === form.supplier.toLowerCase())
      if (exact) {
        commitSupplierSelection(exact)
      }
      else {
        focusCell(0, 'desc')
      }
  ...
  return (
    <div className="erp-workspace-shell erp-workspace-shell--invoice erp-workspace-shell--purchase">
      <div className="erp-workspace erp-invoice-workspace erp-invoice-workspace--purchase">
          ...
          <div className="erp-header-strip">
            <div className="erp-header-party">
              <div style={{ position: 'relative' }}>
                <Input
                  ref={setHeaderRef(0)}
                  data-page-focus="purchase-supplier"
                  ...
                  <div className="erp-dropdown">
                    {suggestions.map((party, index) => (
                      <button
                        key={party.id}
                        ref={(node) => {
                          supplierOptionRefs.current[index] = node
                        }}
                        type="button"
            ...
            <div className="erp-header-meta">
              <Input ref={setHeaderRef(2)} label="Purchase Date" type="date" value={form.date} onChange={(event) => setField('date', event.target.value)} onKeyDown={consumeHeaderEnter(2)} inputClassName="erp-field" />
              ...
              <Input ref={setHeaderRef(4)} label="Supplier Bill No." value={form.billNo} onChange={(event) => setField('billNo', event.target.value)} onKeyDown={consumeHeaderEnter(4)} inputClassName="erp-field" placeholder="Vendor bill ref." />
              <Select ref={setHeaderRef(5)} label="Document Type" value={form.purchaseType} onChange={(event) => setField('purchaseType', event.target.value)} options={['Purchase Bill', 'Debit Note']} onKeyDown={consumeHeaderEnter(5)} selectClassName="erp-field" />
              <Input ref={setHeaderRef(6)} label="Contact" value={form.contactPerson} onChange={(event) => setField('contactPerson', event.target.value)} onKeyDown={consumeHeaderEnter(6)} inputClassName="erp-field" />
              <Input ref={setHeaderRef(7)} label="Phone" value={form.phone} onChange={(event) => setField('phone', event.target.value)} onKeyDown={consumeHeaderEnter(7)} inputClassName="erp-field" />
              <Input ref={setHeaderRef(8)} label="City" value={form.city} onChange={(event) => setField('city', event.target.value)} onKeyDown={consumeHeaderEnter(8)} inputClassName="erp-field" />
              <Input ref={setHeaderRef(9)} label="GSTIN" value={form.gstin} onChange={(event) => setField('gstin', event.target.value.toUpperCase())} onKeyDown={consumeHeaderEnter(9)} inputClassName="erp-field" />
            </div>

            <div className="erp-header-actions">
              <div className="erp-shortcut-box">
                <div>Ctrl + Enter</div>
                <div>Save Purchase</div>
              </div>
              <div className="erp-shortcut-box">
        ...
        <section className="erp-workspace-band erp-workspace-band--footer">
          <div className="erp-footer-grid">
            <div className="erp-footer-panel">
              <div className="erp-footer-title">Receipt & narration</div>
              <div className="erp-footer-fields erp-footer-fields--three">
                <Input ref={setFooterRef(0)} label="Vehicle / LR No." value={transport.vehicleNo} onChange={(event) => setTransport((current) => ({ ...current, vehicleNo: event.target.value }))} onKeyDown={consumeFooterEnter(0)} inputClassName="erp-field" />
                <Input ref={setFooterRef(1)} label="Goods Received At" value={transport.dispatchFrom} onChange={(event) => setTransport((current) => ({ ...current, dispatchFrom: event.target.value }))} onKeyDown={consumeFooterEnter(1)} inputClassName="erp-field" />
                <Select ref={setFooterRef(2)} label="Payment Terms" value={transport.dispatchThrough} onChange={(event) => setTransport((current) => ({ ...current, dispatchThrough: event.target.value }))} options={['Credit', 'Cash', 'Bank', 'UPI', 'Against GRN']} onKeyDown={consumeFooterEnter(2)} selectClassName="erp-field" />
              </div>
              <Textarea
                ref={setFooterRef(3)}
                ...
                placeholder="Terms, narration, inward reference"
                onKeyDown={consumeFooterEnter(3)}
                textareaClassName="erp-field erp-field--textarea"
              />
            </div>

            <div className="erp-footer-panel erp-footer-panel--totals erp-footer-panel--purchase-totals">
              <div className="erp-footer-title">Amount Summary</div>
              <div className="erp-tax-block">
                {taxBreakdown.length > 0 ? taxBreakdown.map((row) => (
                  <div key={row.rate} className="erp-tax-block-row">
                    <span>GST @ {row.rate}% (on taxable {fmt(Math.round(row.taxable))})</span>
                  ...
                  <div className="erp-tax-block-row erp-tax-block-row--muted">
                    <span>No GST lines yet</span>
                    <span className="erp-tax-block-value">—</span>
                  </div>
                )}
              </div>
              <div className="erp-summary-table">
                ...
                <SummaryRow label="Bill Total" value={fmt(total)} large />
              </div>
...
const ERP_PRIMARY_BUTTON_PURCHASE = {
  ...ERP_ACTION_BUTTON,
  background: '#5c3d1e',
}
